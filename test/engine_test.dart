...
void main() {
  test('resampling keeps endpoints and handles duplicate or stationary points',
      () {
    final points =
        resample([const Point2(0, 0), const Point2(0, 0), const Point2(1, 0)]);
  ...
  test('translation, scale and sampling speed do not alter matching', () {
    final original = sample(List.generate(
        60,
        (i) => Point2(
            math.cos(i / 59 * 2 * math.pi), math.sin(i / 59 * 2 * math.pi))));
    final transformed = sample(List.generate(90, (i) {
      final angle = math.pow(i / 89, 2) * 2 * math.pi;
      return Point2(10 + 3 * math.cos(angle), -7 + 3 * math.sin(angle));
    }));
    final result = GestureRecognizer()
        .recognize(transformed, [GestureTemplate('circle', original)]);
  ...
  test('direction remains significant and finger count must agree', () {
    ...
    expect(
        GestureRecognizer()
            .recognize(sample(line(0, reverse: true)), [template]).accepted,
        isFalse);
    expect(
        GestureRecognizer()
            .recognize(GestureSample([line(0), line(1)]), [template]).name,
        isNull);
    expect(
        GestureRecognizer().recognize(sample(line(0)), []).accepted, isFalse);
  });
  test('finger permutation matches but relative geometry is preserved', () {
    final template =
        GestureTemplate('parallel', GestureSample([line(0), line(0.5)]));
    final recognizer = GestureRecognizer();
    expect(
        recognizer.recognize(
            GestureSample([line(0.5), line(0)]), [template]).distance,
        closeTo(0, 1e-9));
    expect(
        recognizer
            .recognize(GestureSample([line(0), line(4)]), [template]).accepted,
        isFalse);
  });
  test('contact IDs prevent crossing fingers from swapping tracks', () {
    ...
    for (var i = 0; i < 20; i++) {
      capture
          .addFrame({9: Point2(i / 19, 0), 2: Point2(1 - i / 19, 0)}, i * 16);
    }
  ...
  test('partial lift waits for all fingers; stationary fingers are retained',
      () {
    final capture = GestureCapture();
  ...
  test(
      'silence, stale frames, cancellation and taps never create false matches',
      () {
    final capture = GestureCapture();
    ...
    expect(capture.addFrame({}, 910), isNull);
  ...
  test('excessive contacts and long sessions are discarded until release', () {
    final capture = GestureCapture();
    capture
        .addFrame({for (var i = 0; i < 6; i++) i: Point2(i.toDouble(), 0)}, 0);
  ...
  test('completed samples are immutable and independent of future captures',
      () {
    final capture = GestureCapture();
